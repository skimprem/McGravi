...
integer function Rempli_Mc_Cal(MC,type_calc)
    ...
    do k = 1, Mc%Nb_obsAbs
        row = pos(k)
        MC%A(MC%Nb_obsRel+k,row)=1.0D0 
        MC%P(MC%Nb_obsRel+k)= 1.0D0 / DBLE(TabObsAbs(k)%sd)**2
        MC%B(MC%Nb_obsRel+k) = TabObsAbs(k)%grav - MC%X0(row) 
        MC%pos(k)=pos(k)  
...
subroutine create_Mc(MC,type_calc)
    ...
    IF (DEBUG) WRITE(0,*)'CREATE_MC : START'
    ...
    IF (DEBUG) WRITE(0,*)'CREATE_MC : deallocating matrices'
    ...
    ! Allocation des matrices *************************************************
    IF (DEBUG) WRITE(0,*)'CREATE_MC : allocating new matrices'
    if (type_calc==1) then
    ...
    ! Remplissage du modèle pour les observations relatives *******************
    IF (DEBUG) WRITE(0,*)'CREATE_MC : filling matrices'
    IF (DEBUG) WRITE(0,*)'CREATE_MC : relative observations'
    do k=1,param%Nb_obsRel
        !if(mod(k,100)==0)write(0,*)'k =',k
    ...
    !do k = 1, Mc%Nb_obsAbs
    !	write(0,*)TabObsAbs(k)%nomsta,TabObsAbs(k)%grav,TabObsAbs(k)%sd,TabObsAbs(k)%num_abs_file
    !end do
    
    ! Ajout des observations absolues *****************************************
    ! on ajoute la première observation absolue (suppression du défaut de rang) 
    IF (DEBUG) WRITE(0,*)'CREATE_MC : absolute observations'
    do k = 1, Mc%Nb_obsAbs
        row = pos(k)
        !write(0,*)row,TabObsAbs(k)%grav,TabObsAbs(k)%sd
        MC%A(MC%Nb_obsRel+k,row)=1.0D0 
        MC%P(MC%Nb_obsRel+k)= (1.0D0 / DBLE(TabObsAbs(k)%sd))**2
        MC%B(MC%Nb_obsRel+k) = TabObsAbs(k)%grav
        MC%pos(k)=pos(k)      
    end do
    ...
    ! Normalisation par enrichissement
    IF (DEBUG) WRITE(0,*)'CREATE_MC : Calculating normal equations'
    CALL Normalise(MC)
    
    IF (DEBUG) WRITE(0,*)'CREATE_MC : writing debug matrices'
    IF (param%writemat) THEN
        !CALL write_mat(MC%A,MC%Nb_obsRel+1,MC%Nb_inc,'Matrice_A.txt')
        CALL write_oct_mat(MC%A,MC%Nb_obsRel+1,MC%Nb_inc,'A','A.m')
    END IF
    
    ! Allocation de l'inverse de AtPA *****************************************
    IF (DEBUG) WRITE(0,*)'CREATE_MC : allocating tmp matrices (MC%sx, MC%sig)'
    allocate (MC%sx(param%Nb_inc,param%Nb_inc),MC%sig(param%Nb_inc))

    IF (DEBUG) WRITE(0,*)'CREATE_MC : END'
...
subroutine Normalise(MC)
    ...
    DEBUG = .FALSE.
    IF (DEBUG) WRITE(0,*)'NORMALISE : START'
    ...
    IF (DEBUG) WRITE(0,*)'NORMALISE : relative observations'
    ...
    ! Ajout des observations absolues *****************************************
    ! on ajoute la première observation absolue (suppression du défaut de rang) 
    IF (DEBUG) WRITE(0,*)'NORMALISE : absolute observations'
    do k = 1, Mc%Nb_obsAbs
        row = pos(k)
        ...
        MC%AtPB(row) = MC%AtPB(row) + wg * TabObsAbs(k)%grav   
    end do
    
    if (param%writemat) then
        IF (DEBUG) WRITE(0,*)'NORMALISE : writing debug matrices'
        CALL write_oct_mat(MC%AtPA,MC%Nb_inc,MC%Nb_inc,'AtPA','AtPA_enrichissement.m')
        CALL write_oct_mat(MC%AtPB,MC%Nb_inc,1,'AtPB','AtPB_enrichissement.m')
    ...
    IF (DEBUG) WRITE(0,*)'NORMALISE : END'
...
subroutine Normalise_cal(MC)
    ...
    ! boucle ne faisant rien mais temporise pour éviter un plantage lors de l'allocation
    m = 0  
    !do l=1,100000000000
    do l=1,   2147483647
        m = m +1 
    ...
    do k = 1, Mc%Nb_obsAbs
        ...
        MC%AtPB(row) = MC%AtPB(row) + wg * (TabObsAbs(k)%grav - MC%X0(row))
    end do
    ...
    if (param%writemat) then
        ...
        CALL write_oct_mat(MC%AtPA,MC%Nb_inc,MC%Nb_inc,'AtPACal','AtPACal_enrichissement.m')
        CALL write_oct_mat(MC%AtPB,MC%Nb_inc,1,'AtPBCal','AtPBCal_enrichissement.m')
    end if
...
subroutine AtPA_AtPB_pousselabrouette(MC)
    ...
    if (param%writemat) then
        ...
        CALL write_oct_mat(MC%AtPA,MC%Nb_inc,MC%Nb_inc,'AtPACal','AtPACal_pousse_la_brouette.m')
        CALL write_oct_mat(MC%AtPB,MC%Nb_inc,1,'AtPBCal','AtPBCal_pousse_la_brouette.m')
    end if
