...
! Procedure qui calcule les résidus standards ou normalisés (au choix de l'utilisateur)
...
integer function Calc_resid(MC,mode)
    ...
    ! le calcul des résidus std est très gourmand.
    ! On ne le fait que si c'est demandé
    ...
    ! Test JBL pour verifier le comportement du résidu sur l'unique obs absolue dans le cas d'une solution libre
...
integer function statistique(MC,mode)
    ...
    if (mode==1) then
        nb_oa = 0; 
        ! on force à 0 (il y en a une en fait) car dans le cas d'une solution libre 
        ! l'unique obs absolue sert à rendre le système inversible
        ! par contre ça n'a pas vraiment de sens d'en tenir compte 
    ...
    if (param%lg=='F') then
            write(67,'(1x,A25,I4,A9)')'Histogramme des résidus (',&
    ...
    do i=-9,8 ! Conversion des valeurs de l'histogramme en valeurs dessinables
        ...
        if (hist(i+10)>47) hist(i+10)=47 ! on tronque le dessin à 47
    end do
...
!Code traduit à partir d'un code Fortran 77 écrit par C. Hwang  pour le programme Gravnet
...
! dessin d'un histogramme des résidus
    ...
    if (param%lg=='F') then ; ch = 'fréquence relative' ;  else ; ch = 'relative frequency' ; end if
    do i = 1,18
	    dessin(29-i,1) = ch(i:i)
    end do
    
    if (param%type_resid) then
        if (param%lg=='F') then ; ch = 'résidus standards ' ;  else  
        ...
        if (param%lg=='F') then ; ch = 'résidus normalisés' ;  else  
                                  ch = 'Standard residuals' ; end if
        do i = 1,18
	        dessin(1,i+30) = ch(i:i)
        end do
